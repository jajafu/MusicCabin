...
- GMS builds show only Photo frame 2 in the main menu; the original frame remains implemented with its data preserved. It opens directly into the saved local-photo source or Drive folder; the gear groups source choices, album actions, interval, crop, clock, and song display in rounded cards with a fixed header and Done button, while Drive reuses the shared image cache and supports cloud pause/resume. The overlay provides separate song previous/play-pause/next controls plus photo previous/next, gear, and exit; see the [Google Drive test guide](docs/photo-frame-google-drive-testing.zh-TW.md).
