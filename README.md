...
- On Google TV and Android TV, Photo frame 1 serves a local photo picker page while its transfer settings are open. Scan the TV's QR code or open its address on a phone using the same Wi-Fi, then choose photos from the phone or a cloud provider shown in the system file picker. No pairing code is required. The phone compresses each photo to at most 1920 pixels on the longest edge and sends it directly to the TV. Transfers accumulate; the TV can play or clear received copies without changing originals. Both FOSS and GMS TV builds include this frame without TV Google OAuth or an external picker website. The TV settings use a fixed dialog for local photos, phone transfer, and display controls. Previously selected TV Photo frame 2 local photos and received copies are imported to Photo frame 1 on first launch after upgrading; FOSS and GMS app data remain separate. Phone and head-unit Photo frame 2 keep their original Google Drive OAuth and folder playback.
- On Google TV and Android TV, the local photo browser focuses Allow access when permission is needed and shows a clear white outline around remote-selected frame settings and browser controls. After permission is granted, focus returns to the photo browser toolbar. The Internal storage tab identifies the TV's built-in storage; an empty gallery means no accessible indexed photos were found, and the TV shows guidance for that case instead of USB instructions.
- The TV Home shows high contrast category headings and a compact selection of songs per category, so more recommendations appear sooner. Its remote menu uses equal width, left aligned items with clear focus states.
- The TV menu's Stop and exit button ends playback and closes the playback service. Ordinary Back keeps music playing in the background.
- Google TV and Android TV open a separate remote-control interface, including when the standard launcher icon is used. Use the D-pad and Select to browse quick picks and online songs grouped under their original Home section names, with cover art in Home, search, and the playback queue. Search songs with the TV keyboard, control playback, and open or play from the queue. The TV menu opens Photo frame 1 with local photos or copies sent from a phone, slideshow controls, and settings operated by remote. Its translucent clock, song information, and controls follow the existing frame settings. Select the fullscreen icon or press Menu to hide controls; D-pad or Select brings them back. Back closes frame settings first, reveals hidden controls, then returns to TV Home. Phones and Android head units retain the existing touch interface. Other TV settings and the full phone feature set are not included in this first TV version.
...
- GMS builds show only Photo frame 2 in the phone and head-unit main menu; the original frame remains implemented with its data preserved. It opens directly into the saved local-photo source or Drive folder; Drive validates up to five randomized cached photos for immediate offline playback, expands to the complete intact cache in the background, then keeps playing while OAuth and cloud metadata refresh concurrently. A successful refresh seeds the visible photo plus four randomized upcoming photos and downloads only cache misses. Authorization or network failures appear with a stable error code below the controls while cached photos continue rotating offline. The gear groups source choices, album actions, interval, crop, clock, and song display in rounded cards with a fixed header and Done button, and supports cloud pause/resume. The overlay provides separate song previous/play-pause/next controls plus photo previous/next, gear, and exit; see the [Google Drive test guide](docs/photo-frame-google-drive-testing.zh-TW.md).
...
- Uses separate Android package IDs for the public FOSS app (`com.jajafu.musiccabin`) and self-installed GMS app (`com.jajafu.musiccabin.gms`), allowing both MusicCabin variants and the original Metrolist app to be installed at the same time.
...
- Automatic queue continuation retries temporary failures with bounded backoff, waits for network recovery without duplicate requests, and offers a manual retry after the final failure. When a YouTube continuation is exhausted or keeps returning already queued songs, enabled similar content tries randomly selected, unused song seeds, excludes songs already in the queue even when YouTube returns another video ID for the same title and artist, and shuffles fresh radio additions. If no new songs are available, automatic additions stop instead of appending the old list. End-of-queue checks follow the actual shuffled playback order, and YouTube continuation state survives an App service restart.
...
The in-app updater is enabled only in FOSS builds. It checks [this repository's releases](https://github.com/jajafu/MusicCabin/releases) and opens the matching APK download for confirmation; Android still requires the user to approve installation. Self-installed GMS builds are updated manually.
...
Starting with version `13.7.11`, FOSS keeps `com.jajafu.musiccabin` (Debug: `com.jajafu.musiccabin.debug`) while GMS uses `com.jajafu.musiccabin.gms` (Debug: `com.jajafu.musiccabin.gms.debug`). The two Release variants therefore install side by side, with independent settings, login, downloads, database, permissions, widgets, and shortcuts. A GMS installation that previously used `com.jajafu.musiccabin` does not upgrade into the new package or migrate its data; configure the new installation and register its actual package ID plus signing SHA-1 as a separate Android OAuth client before reconnecting Drive. Both variants still claim some of the same web links and the Discord callback scheme, so Android may ask which app should open them.

Versions `13.6.77`–`13.7.10` used `com.jajafu.musiccabin` for both Release variants, while versions `13.6.52`–`13.6.76` used `com.jajafu.metrolist.androidcar`. Package-ID changes always install as a separate App even with the same signing key. The database schema remains unchanged; later releases with the same variant package ID and signing key can update that variant in place.
