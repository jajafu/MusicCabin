...
本文件涵蓋 [Google Drive 相框規畫](photo-frame-google-drive-plan.zh-TW.md) 的 OAuth／讀取、資料夾播放、離線快取相簿與相框 2 介面驗證。2026-09-12 使用者確認當時版本可在手機運作；13.7.11 將 GMS 改為可與 FOSS 並存的獨立套件，13.7.12 新增 OAuth 前先播放快取與驗證失敗持續離線輪播。主選單／設定介面仍待重新檢查，車機尚未驗證，舊相框選取資料的自動遷移仍未實作。
...
2026-09-17（13.7.12）最終開發驗證：GMS／Foss Debug APK 建置及 94 項新舊相框單元測試通過（0 失敗、0 錯誤、0 跳過），`git diff --check` 通過。新增測試涵蓋持久化離線索引、舊快取 metadata 比對、隨機首批 5 張、取消背景掃描後立即重新進入、純快取切換雲端清單，以及雲端僅剩一張不同照片時不保留錯誤舊畫面。實際地下室斷網、OAuth 失效與長時間車機離線輪播仍待目標裝置驗收。
...
3. 確認 Drive 自動隨機全螢幕播放：預設 10 秒，可選 5／10／15／30／60 秒；從齒輪測試雲端暫停／繼續，控制層測試歌曲上一首／播放暫停／下一首與照片上一張／下一張。第一張先顯示，再預載接下來 4 張隨機壓縮檔，使首批共 5 張可離線播放；本機來源則確認原有自動輪播，不套用雲端預載。
4. 播放與預載先查共用圖片快取，缺少或版本變更時才單工下載；不預先下載整個資料夾。顯示解碼長邊不超過 1920px，預載檔案不解碼。暫存檔於載入後清理，已提交的圖片保留於共用快取並記入可離線列舉的索引。
5. 進背景或離開相框 2 頁面後會停止工作；再次進入 Drive 來源時，先從索引隨機驗證最多 5 張快取並開始播放，再於背景加入其餘有效快取，同時靜默驗證 OAuth、帳戶及保存的資料夾。成功後保留目前畫面並換成最新清單；授權過期或無網路時，操作列下方顯示訊息與穩定錯誤碼，快取輪播不中斷。舊相框的來源與選取資料應保持獨立。
6. 重播快取照片時應避免再次下載。圖片容量與清除操作沿用「設定 → 儲存空間」；容量為 0 時不保留照片，也不預載。清除後，再次播放會重新下載。
7. 測試 token 更新；切換帳戶前先本機解除連接。輪播發生網路／授權等整體錯誤時停止新下載，繼續隨機播放全部有效快取照片；從齒輪的雲端暫停／繼續可重試下載，需要重新授權則手動重新連接。

從 13.7.11 或更舊版本升級時，既有共用快取只有圖片二進位資料，沒有可離線反查的 Drive metadata。安裝 13.7.12 後請先在有網路處成功更新一次保存的 Drive 資料夾；App 會在背景比對 metadata 並把已存在的舊快取加入離線索引，不會為命中的圖片重新下載。完成一次後再測完全離線啟動。
...
| 網路錯誤 | 操作列下方顯示訊息與 `NETWORK` 等穩定錯誤碼並可重試；既有快取持續輪播，不把 YouTube Music 登出。 |
...
| 快取 | 重新進入時先隨機驗證最多 5 張並播放，再於背景擴充為全部有效快取；雲端播放首批目前照片加 4 張隨機預載共 5 張。重播不再次下載圖片；不同帳戶、檔案版本分開。快取清除／淘汰後會從離線索引排除並在連線後按需下載，損壞與取消不提交半檔。升級舊快取需先成功更新一次資料夾以建立索引。 |
...
| 進背景／返回 | 關閉播放並釋放解碼圖片與下載工作，保留來源選擇與快取；返回後本機來源自動恢復，Drive 則先恢復快取輪播再背景驗證，過期授權顯示錯誤碼且快取不中斷，重新連接仍從齒輪操作。 |
