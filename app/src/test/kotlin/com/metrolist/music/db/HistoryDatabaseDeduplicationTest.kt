...
class HistoryDatabaseDeduplicationTest {
    ...
    @Test
    fun `history hydrates only the latest play per song in each displayed period`() =
        runBlocking {
    ...
    @Test
    fun `repeated plays do not create repeated song relation graphs`() =
        runBlocking {
            repeat(2_000) { index ->
                insertEvent(
                    id = index.toLong() + 1,
    ...
    @Test
    fun `latest event lookup stays bounded to the newest play`() =
        runBlocking {
            insertEvent(id = 1, songId = SONG_ID, timestamp = TODAY.atTime(9, 0))
