...
- Google TV／Android TV 的「數位相框 1」會在傳送設定開啟時提供區域網路選檔網頁。手機連上同一 Wi-Fi，掃描電視顯示的 QR code 或開啟網址，即可從手機或系統檔案選擇器可見的雲端來源挑照片，不需配對碼。手機先將照片縮至最長邊 1920 像素並壓縮，再直接傳到電視。照片會累加，TV 可播放或清空接收副本，不影響原檔；FOSS 與 GMS 的 TV 版都可使用，不需要 TV Google OAuth 或外部選檔網站。TV 設定以固定視窗分別顯示本地照片、手機傳送與顯示選項。升級後首次開啟 TV 相框 1 時，會匯入先前 TV 相框 2 選取的本地照片與接收副本；FOSS 與 GMS 的 App 資料仍各自獨立。手機與車機的相框 2 保留原本的 Google Drive OAuth 與資料夾播放。
- Google TV／Android TV 的本地照片瀏覽器需要授權時會先選中「允許存取」，遙控器選到的相框設定與瀏覽器操作會有清楚的白色外框；授權成功後焦點會回到照片瀏覽器工具列。「內部儲存空間」分頁就是電視內建儲存區；相簿空白表示目前沒有可讀取且已建立索引的照片，TV 會顯示對應提示，不再誤導使用者重插 USB。
- TV 首頁使用高對比分類標題，並精簡每類顯示的歌曲數，讓更多推薦分類較快出現；遙控器側欄改為等寬、靠左對齊的選項與清楚的焦點提示。
- TV 左側選單的「停止並退出」會結束播放並關閉播放服務；一般返回仍會讓音樂在背景播放。
- Google TV／Android TV 會開啟獨立的遙控器介面，從一般啟動圖示進入時也會切換至 TV 畫面。可用方向鍵與確認鍵瀏覽歌曲快選與依原始首頁區塊名稱分類的線上歌曲；首頁、搜尋及播放佇列都會顯示歌曲封面。也能透過電視鍵盤搜尋、控制播放，並從佇列選歌。TV 左側選單可開啟「數位相框 1」，使用本地照片或手機傳入的照片副本，並以遙控器操作輪播控制及相框設定。半透明時鐘、歌曲資訊與控制列沿用相框設定；選全螢幕圖示或按選單鍵可隱藏操作列，方向鍵或確認鍵可叫回。返回鍵會先關閉相框設定、叫回已隱藏的控制列，再返回 TV 首頁。手機與 Android 車機仍使用原有觸控介面；其他 TV 設定及手機版完整功能尚未加入。
...
- GMS 建置的手機與車機主選單僅顯示「數位相框 2」，原有相框的功能與資料仍保留。開啟後直接使用保存的本機照片來源或 Drive 資料夾；Drive 會先隨機驗證最多 5 張快取照片並立即離線播放，再於背景擴充為全部有效快取，同時驗證 OAuth 與更新雲端清單。成功更新後會準備目前照片及接下來 4 張隨機照片，且只下載快取缺少的檔案；授權或網路失敗時在操作列下方顯示穩定錯誤碼，快取照片仍持續離線輪播。齒輪以圓角分組整理來源選擇、相簿操作、間隔、裁切、時鐘與歌曲顯示，並固定顯示標題與完成按鈕，也支援雲端暫停／繼續。控制層第二列分開提供歌曲上一首／播放暫停／下一首、照片上一張／下一張、齒輪與退出；請參閱 [Google Drive 測試指南](docs/photo-frame-google-drive-testing.zh-TW.md)。
- 將播放島放大 2 倍，提升資訊可見度。
...
- 自動延伸播放佇列遇到暫時性錯誤時會以有限退避重試，斷線時等待網路恢復且不重複請求，最終失敗後可手動重試。YouTube 的續頁結束或持續回傳已在佇列中的歌曲後，若已啟用類似內容，會隨機挑選尚未用過的歌曲作為電台來源；即使 YouTube 用不同影片 ID 回傳相同歌名與歌手，也會排除舊佇列歌曲，並打散新推薦的順序。找不到新歌時會停止自動延伸，不會接上重複的舊清單。佇列尾端判斷會依照隨機播放的實際順序，App 播放服務重啟後也會保留 YouTube 續頁狀態。
