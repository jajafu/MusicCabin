...
- GMS 建置的主選單僅顯示「數位相框 2」，原有相框的功能與資料仍保留。開啟後直接使用保存的本機照片來源或 Drive 資料夾；齒輪以圓角分組整理來源選擇、相簿操作、間隔、裁切、時鐘與歌曲顯示，並固定顯示標題與完成按鈕，Drive 則沿用共用圖片快取並支援雲端暫停／繼續。控制層第二列分開提供歌曲上一首／播放暫停／下一首、照片上一張／下一張、齒輪與退出；請參閱 [Google Drive 測試指南](docs/photo-frame-google-drive-testing.zh-TW.md)。
- 將播放島放大 2 倍，提升資訊可見度。
